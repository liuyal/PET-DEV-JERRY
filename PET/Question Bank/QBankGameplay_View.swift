//
//  QBankGameplay_View.swift
//  PET
//
//  Created by liuyal on 11/13/17.
...
class QBankGameplay_View: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        QuestionImage.image = user?.CustomQArray[index!].image
        ...
        for i in 1..<DeckSize+1{
            ...
            if cardObj.emotion == self.user?.CustomQArray[index!].emotion{
    ...
    @IBAction func Action2(_ sender: UIButton) {
        ...
        if selected == user?.CustomQArray[index!].emotion {
            
            // Change question state to correct
            user?.CustomQArray[index!].questionState = .correct;
            ...
            if self.index! >= (user?.CustomQArray.count)! {
        ...
        else if selected != user?.CustomQArray[index!].emotion{
            ...
            user?.CustomQArray[index!].questionState = .incorrect
            user?.CustomQArray[index!].errCnt += 1
            self.updateProgressDB()
            print("ERROR")
        }
        else {
    ...
    @IBAction func backbutton(_ sender: UIButton) {
        self.performSegue(withIdentifier: "segQBankback", sender: self)
    }
    ...
    func updateProgressDB() {
        ...
        let id = self.user?.ID
        let err = self.user?.CustomQArray[index!].errCnt
        
        let Slevel = user?.CustomQArray[index!].levelID
        let Squestion = user?.CustomQArray[index!].questionID
        let A = String(Slevel!)
        let B = String(Squestion!)
        let Qname = A + B
        
        var Instate: Int
        if self.user?.CustomQArray[index!].questionState == .skipped {  Instate = 3 }
        else if self.user?.CustomQArray[index!].questionState == .incorrect {  Instate = 2 }
        else if self.user?.CustomQArray[index!].questionState == .correct { Instate = 1 }
        else { Instate = 0}
        
        self.ref.child("ROOT").child(id!).child("CustomQuestions").child(Qname).child("State").setValue(Instate)
        self.ref.child("ROOT").child(id!).child("CustomQuestions").child(Qname).child("errCnt").setValue(err)
    }
    ...
    func updatePage(indexx: Int){
        ...
        QuestionImage.image = user?.CustomQArray[indexx].image
        
        titbar.text = user?.CustomQArray[indexx].imageFileRef
        ...
        for i in 1..<DeckSize+1{
            ...
            if cardObj.emotion == self.user?.CustomQArray[indexx].emotion{
                buttonCards.append(cardObj)
            }
            else{
                cardsArray.append(cardObj)
